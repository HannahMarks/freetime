...
- 🚧 Recurring availability (v1 — v6) — `recurrence_rule` JSONB on both `busy_blocks` and `unavailable_days`, with `expandOccurrences()` walking each series at +7d intervals (DST-safe via `setDate`); `listCalendarItems` returns one CalendarItem per occurrence in the requested window. Rule shape: `{freq:'weekly', byDay?: number[], until?: 'YYYY-MM-DD'}`. AddItemSheet (busy + unavailable modes) has a "Repeat weekly" toggle, 7 day-of-week chips that auto-seed the base's weekday, and an "Ends on a date" sub-toggle. View mode summarises as "Weekly on Mon, Wed, Fri until Dec 31, 2026". v4 adds per-occurrence skip exceptions on busy_blocks (`busy_block_exceptions` table, action='skip'|'move'); the three-dots popover splits into "Delete this occurrence" + "Delete entire series" for recurring busy_blocks. v5 adds drag-to-move-just-this-one for busy_blocks: dragging a recurring occurrence writes a `move` exception so siblings stay put; `CalendarItem.originalStart` carries the pre-move timestamp so subsequent edits find the right exception row. v6 ships the parallel `unavailable_day_exceptions` table (action='skip'|'move' with composite PK `series_user_id, series_date, original_date`); recurring unavailable_days now get the same "Delete this occurrence" / "Delete entire series" popover split via `skipUnavailableDayOccurrence`
