...
- 🚧 Comments schema + UI (P4c) — `comments(post_id, author_id, body, created_at)` table with author-FK to `profiles`. RLS gates visibility through a new `is_post_visible(post_id)` SECURITY DEFINER helper (parallel to `is_friend_of` — comments are visible iff the parent post is). INSERT pinned to `auth.uid()` and only on visible posts; DELETE allowed for comment author OR post author (moderation). New `lib/comment-actions.ts` (`listPostComments`, `createComment`, `deleteComment`). UI: each feed row gets a "💬 Comment" toggle → expands an inline `PostComments` thread (chronological, oldest-first) with its own compose row + per-comment trash for the comment's own author / the post's host. Multiple threads can be open at once
- ⏳ Likes schema + UI (P4d)
