...
- 🚧 Recurring availability (v1 — v7) — `recurrence_rule` JSONB on both `busy_blocks` and `unavailable_days`, with `expandOccurrences()` walking each series at +7d intervals (DST-safe via `setDate`); `listCalendarItems` returns one CalendarItem per occurrence in the requested window. Rule shape: `{freq:'weekly', byDay?: number[], until?: 'YYYY-MM-DD'}`. AddItemSheet (busy + unavailable modes) has a "Repeat weekly" toggle, 7 day-of-week chips that auto-seed the base's weekday, and an "Ends on a date" sub-toggle. View mode summarises as "Weekly on Mon, Wed, Fri until Dec 31, 2026". v4 adds per-occurrence skip exceptions on busy_blocks; the three-dots popover splits into "Delete this occurrence" + "Delete entire series" for recurring busy_blocks. v5 adds drag-to-move-just-this-one for busy_blocks via `move` exceptions; `CalendarItem.originalStart` carries the pre-move timestamp. v6 ships the parallel `unavailable_day_exceptions` table; recurring unavailable_days get the same popover split. v7 adds **edit-just-this-one**: exception tables now carry override metadata columns (title/notes/location), and editing a recurring occurrence's form prompts via Alert: "Save changes to → This event only / Entire series / Cancel". "This event only" upserts a `move` exception layering the override values on top of the series's; "Entire series" mutates the base row as before
