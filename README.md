...
- 🚧 Recurring availability (v1 + v2 + v3 + v4) — `recurrence_rule` JSONB on both `busy_blocks` and `unavailable_days`, with `expandOccurrences()` walking each series at +7d intervals (DST-safe via `setDate`); `listCalendarItems` returns one CalendarItem per occurrence in the requested window. Rule shape: `{freq:'weekly', byDay?: number[], until?: 'YYYY-MM-DD'}`. AddItemSheet (busy + unavailable modes) has a "Repeat weekly" toggle, 7 day-of-week chips that auto-seed the base's weekday, and an "Ends on a date" sub-toggle. View mode summarises as "Weekly on Mon, Wed, Fri until Dec 31, 2026". v4 adds per-occurrence skip exceptions on busy_blocks (`busy_block_exceptions` table, action='skip'|'move'); the three-dots popover splits into "Delete this occurrence" + "Delete entire series" for recurring busy_blocks
